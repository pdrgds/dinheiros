...
fn test_parse_b3_xlsx_basic_shape() {
    ...
    assert!(
        result
            .transactions
            .iter()
            .any(|t| t.asset_type == AssetType::StockBr),
    ...
    assert!(
        result
            .transactions
            .iter()
            .any(|t| t.asset_type == AssetType::Tesouro),
    ...
    assert!(
        result
            .income
            .iter()
            .any(|i| i.income_type == IncomeType::Dividend),
    ...
    assert!(
        result
            .income
            .iter()
            .any(|i| i.income_type == IncomeType::Jcp),
    ...
    assert!(
        petr4_div.tax_withheld.is_none(),
        "Dividends are tax-exempt in Brazil"
    );
}
...
fn test_b3_symbol_extraction() {
    assert_eq!(
        extract_symbol("CMIG3 - CIA. ENERGETICA DE MINAS GERAIS"),
        "CMIG3"
    );
...
fn test_b3_liquidacao_credito_classified_as_buy() {
    ...
    assert!(
        (tx.total_value - 453.60).abs() < 0.01,
        "total_value: {}",
        tx.total_value
    );
...
fn test_b3_liquidacao_debito_classified_as_sell() {
    ...
    assert!(
        (tx.total_value - 2003.85).abs() < 0.01,
        "total_value: {}",
        tx.total_value
    );
}
...
fn test_b3_plain_transferencia_dropped() {
    ...
    let on_05_04: Vec<_> = result
        .transactions
        .iter()
        .filter(|t| t.symbol == "BBAS3" && t.date == NaiveDate::from_ymd_opt(2024, 4, 5).unwrap())
        .collect();
    assert!(
        on_05_04.is_empty(),
...
fn test_b3_desdobro_emits_buy_with_zero_cost() {
    ...
    assert_eq!(tx.tx_type, TxType::Buy);
    assert!(
        tx.total_value.abs() < 0.01,
        "Desdobro must have zero cost basis"
    );
}
...
fn test_b3_transferred_symbol_has_buys_and_sell() {
    ...
    assert_eq!(
        buys, 5,
        "expected 5 BBAS3 Buys (4 Liq.Cr + 1 Desdobro), got {}",
        buys
    );
    assert_eq!(
        sells, 1,
        "expected exactly 1 BBAS3 Sell (the Liq.Db), got {}",
        sells
    );
}
