...
fn row_to_transaction(row: &rusqlite::Row) -> Result<Transaction> {
    ...
    Ok(Transaction {
        ...
        asset_type: AssetType::from_str(&asset_type_str).unwrap_or(AssetType::StockBr),
        symbol: row.get(3)?,
        tx_type: TxType::from_str(&tx_type_str).unwrap_or(TxType::Buy),
...
pub fn get_all_income(db: &Database) -> Result<Vec<Income>> {
    ...
    let rows = stmt.query_map([], |row| {
        ...
        Ok(Income {
            ...
            date: NaiveDate::parse_from_str(&date_str, "%Y-%m-%d")
                .unwrap_or_else(|_| NaiveDate::from_ymd_opt(2000, 1, 1).unwrap()),
            income_type: IncomeType::from_str(&income_type_str).unwrap_or(IncomeType::Dividend),
            currency: row.get(5)?,
