...
mod tests {
    ...
    fn every_transacted_symbol_has_a_latest_price() {
        ...
        assert!(missing.is_empty(), "transacted symbols without a price: {:?}", missing);
    }
    ...
    fn income_symbols_match_transacted_symbols() {
        ...
        let orphans: Vec<_> = all_income()
            ...
            .collect();
        assert!(orphans.is_empty(), "income for un-held symbols: {:?}", orphans);
    }
}
